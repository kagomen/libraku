  ...
  const unsubscribe = () => {
    if (signals) {
      ...
      signals.forEach((signal2) => {
        signal2 &&
          (signal2.removeEventListener ? signal2.removeEventListener('abort', onabort) : signal2.unsubscribe(onabort))
      })
      signals = null
    }
  }
...
var fetch_default =
  ...
  (async (config) => {
    ...
    let [composedSignal, stopTimeout] =
      signal || cancelToken || timeout ? composeSignals_default([signal, cancelToken], timeout) : []
      ...
      if (
        ...
        if (_request.body) {
          data = trackStream(
            _request.body,
            DEFAULT_CHUNK_SIZE,
            fetchProgressDecorator(requestContentLength, progressEventReducer_default(onUploadProgress)),
            null,
            encodeText,
          )
        }
      ...
      if (supportsResponseStream && (onDownloadProgress || isStreamResponse)) {
        ...
        response = new Response(
          trackStream(
            ...
            DEFAULT_CHUNK_SIZE,
            onDownloadProgress &&
              fetchProgressDecorator(responseContentLength, progressEventReducer_default(onDownloadProgress, true)),
